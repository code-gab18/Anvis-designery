...
window.addEventListener('scroll', () => {
  document.getElementById('nav')
    .classList.toggle('compact', window.scrollY > 60);
});
...
const rvObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      ...
      e.target.querySelectorAll('.linner')
        .forEach(l => l.classList.add('up'));
    }
  });
}, { threshold: 0.1 });
...
form.addEventListener("submit", async function (e) {
  ...
  try {
    const response = await fetch("https://formspree.io/f/mykvvkrn", {
      method: "POST",
      body: formData,
      headers: {
...
successModal.addEventListener("click", (e) => {
  if (
    ...
    e.target.classList.contains("success-backdrop")
  ) {
    successModal.classList.remove("active");
  }
});
